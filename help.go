...
func printKeyboardShortcuts() {
	...
	shortcuts.WriteString(headerStyle.Render("SEARCH"))
	shortcuts.WriteString("\n")
	shortcuts.WriteString("  ")
	shortcuts.WriteString(keyStyle.Render("/"))
	shortcuts.WriteString("             Fuzzy file finder (search by filename)\n")
	shortcuts.WriteString("  ")
	shortcuts.WriteString(keyStyle.Render("Ctrl+F"))
	shortcuts.WriteString("        Content search (ripgrep - search file contents)\n")
	shortcuts.WriteString("  ")
	shortcuts.WriteString(keyStyle.Render("j/k"))
	shortcuts.WriteString("           Navigate search results\n")
	shortcuts.WriteString("  ")
	shortcuts.WriteString(keyStyle.Render("n/N"))
	shortcuts.WriteString("           Next/previous search match (in results)\n")
	shortcuts.WriteString("  ")
	shortcuts.WriteString(keyStyle.Render("Enter"))
	shortcuts.WriteString("         Open selected file or jump to match\n")
	shortcuts.WriteString("  ")
	shortcuts.WriteString(keyStyle.Render("Backspace"))
	shortcuts.WriteString("     Delete character / Return to search input\n")
	shortcuts.WriteString("  ")
	shortcuts.WriteString(keyStyle.Render("Esc"))
	shortcuts.WriteString("           Cancel search\n\n")

	// File Watching (Phase 3)
	shortcuts.WriteString(headerStyle.Render("FILE WATCHING"))
	shortcuts.WriteString("\n")
	shortcuts.WriteString("  ")
	shortcuts.WriteString(descStyle.Render("• Files auto-reload when changed on disk"))
	shortcuts.WriteString("\n  ")
	shortcuts.WriteString(descStyle.Render("• [WATCHING] indicator shows when active"))
	shortcuts.WriteString("\n  ")
	shortcuts.WriteString(descStyle.Render("• [RELOADED] indicator shows after update"))
	shortcuts.WriteString("\n  ")
	shortcuts.WriteString(descStyle.Render("• Scroll position preserved on reload"))
	shortcuts.WriteString("\n\n")

	// File Types
	shortcuts.WriteString(headerStyle.Render("SUPPORTED FILE TYPES"))
	shortcuts.WriteString("\n")
	shortcuts.WriteString("  ")
	shortcuts.WriteString(keyStyle.Render(".md"))
	shortcuts.WriteString("           Markdown - Glamour rendering with TOC\n")
	shortcuts.WriteString("  ")
	shortcuts.WriteString(keyStyle.Render(".json"))
	shortcuts.WriteString("         JSON - Pretty-print with syntax highlighting\n\n")
...
func getHelpOverlay(width, height int, colorManager *ColorManager) string {
	...
	content.WriteString("  ")
	content.WriteString(keyStyle.Render("/"))
	content.WriteString("         Fuzzy finder (by filename)\n")
	content.WriteString("  ")
	content.WriteString(keyStyle.Render("Ctrl+F"))
	content.WriteString("    Content search (ripgrep)\n")
	content.WriteString("  ")
	content.WriteString(keyStyle.Render("j/k"))
	content.WriteString("       Navigate results\n")
	content.WriteString("  ")
	content.WriteString(keyStyle.Render("Enter"))
	content.WriteString("     Open/jump to match\n")
	content.WriteString("  ")
	content.WriteString(keyStyle.Render("Esc"))
	content.WriteString("       Cancel search\n\n")
	...
	content.WriteString("FILES: .md (Glamour) | .json (Pretty-print)\n")
	content.WriteString("WATCHING: Auto-reload on file changes\n\n")
